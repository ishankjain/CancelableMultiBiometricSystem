...
% end
% 
score1=zeros(size(train1,1),size(train2,1));
% score=zeros(1,1);
for i=1:size(train1,1)
    X=train1(i,:);
    for j=1:size(train2,1)
        Y=train2(j,:);
        score1(i,j)=match_biometric(X,Y);
    end
    disp(i);
end
% 
...
% end
% 
% score=zeros(size(fvtrain1,1),size(fvtest1,1));
% % score=zeros(1,1);
% for i=1:size(fvtest1,1)
%     X=fvtrain1(i,:);
%     for j=1:size(fvtest1,1)
%         Y=fvtest1(j,:);
%         score(i,j)=match_biometric(X,Y);
%     end
%     disp(i);
